...
#                    array bounds handling, loop control, etc
        ...
        .data   
TXT0:   .asciiz  "Num. iterations (0 <= K): "  # text to retrieve k value from user
ENDL:   .asciiz  "\n"                          # newline 
	.text
	
J0:     li   $v0, 4     # load instruction to output text to screen
	la   $a0, TXT0  # load address of text to print
	syscall
	
	li   $v0, 5     # load instruction to read integer, K from user
	syscall 	# read K
	
	move $s3, $v0	# move K into saved temporary register
	
	blt  $s3, $0, J0 # if less than 0, branch to J0 and ask again
	beqz $s3, EXIT   # if 0 end program
	
	li   $v0, 4     # load instruction to print newline
	la   $a0, ENDL  # load address newline
	syscall         # print
	
	move $s4, $zero # set up variable to increment in loop
...
L0:    la   $a0, TXT1        # load address of text prompt for syscall
       ...
       bgeu $v0, $s1, L0       # single instruction to check both bounds
...
L1:     lw   $t0, 0($s0)      # load F[i-2] (initially, $s0+0 == &F[0])
        ...
        bgt $s2, $0, L1       # while (0 < M), repeat computation
...
L2:     addi $a0, $0, ' '     # set ASCII for space
        ...
        bgt $t1, $0, L2          # while not finished, repeat computation
        ...
        addiu $s4, $s4, 1     # increment $s4 (counter)
        bgt  $s3, $s4, J1     # branch back to J1 if the counter is less than K
