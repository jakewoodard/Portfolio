...
def F(x):
    ...
    F[0] = R[0] - x[0]/(1+x[1]*sin(theta[0] + x[2]))
    F[1] = R[1] - x[0]/(1+x[1]*sin(theta[1] + x[2]))
    F[2] = R[2] - x[0]/(1+x[1]*sin(theta[2] + x[2]))
    return F

# Initial guess
x = np.array([C, e, alpha])  # What list needs to be used to initialze the Newton-Raphson (N-R) method?
